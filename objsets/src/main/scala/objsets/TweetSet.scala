...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    throw new java.util.NoSuchElementException("Sorry, there is no tweet")
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def compareTweets(tweet1: Tweet, tweet2: Tweet): Tweet = {
      if (tweet1.retweets > tweet2.retweets) tweet1 else tweet2
    }

    if (left.isEmpty && right.isEmpty) elem
    else if (right.isEmpty) compareTweets(left.mostRetweeted,elem)
    else if (left.isEmpty) compareTweets(right.mostRetweeted,elem)
